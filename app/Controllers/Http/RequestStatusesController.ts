...
import RequestStatusesRepository from 'App/Repositories/RequestStatusesRepository'
import { getErrors } from 'App/Services/MessageErros'
import { RequestStatusSchema, RequestStatusSearchSchema } from 'App/Validators'

export default class RequestStatusesController {
  private readonly repository
  constructor () {
    this.repository = RequestStatusesRepository
  }
  ...
  async store ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: RequestStatusSchema})
    } catch (error) {
  ...
  async search ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: RequestStatusSearchSchema})
    } catch (error) {
      const msg = getErrors(error)
      // console.log(error.messages.errors)
      return response

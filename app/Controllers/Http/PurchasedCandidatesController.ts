...
import { PurchasedCandidateSchema, PurchasedCandidateSearchSchema } from 'App/Validators'

export default class PurchasedCandidatesController {
  private readonly repository
  constructor () {
    this.repository = PurchasedCandidatesRepository
  }
  ...
  async search ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: PurchasedCandidateSearchSchema})
    } catch (error) {
      const msg = getErrors(error)
      // console.log(error.messages.errors)
      return response

...
import { PartnerSchema, PartnerSearchSchema } from 'App/Validators'
...
export default class PartnersController {
  ...
  async store ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: PartnerSchema})
    } catch (error) {
  ...
  async search ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: PartnerSearchSchema})
    } catch (error) {
      const msg = getErrors(error)
      // console.log(error.messages.errors)
      return response

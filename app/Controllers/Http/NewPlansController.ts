...
import { NewPlanSchema, NewPlanSearchSchema } from 'App/Validators'

export default class NewPlansController {
  ...
  async store ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: NewPlanSchema})
    } catch (error) {
  ...
  async search ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: NewPlanSearchSchema})
    } catch (error) {
      const msg = getErrors(error)
      // console.log(error.messages.errors)
      return response

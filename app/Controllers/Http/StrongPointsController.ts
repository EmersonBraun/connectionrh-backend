...
import StrongPointsRepository from 'App/Repositories/StrongPointsRepository'
import { getErrors } from 'App/Services/MessageErros'
import { StrongPointSchema, StrongPointSearchSchema } from 'App/Validators'

export default class StrongPointsController {
  private readonly repository
  constructor () {
    this.repository = StrongPointsRepository
  }
  ...
  async store ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: StrongPointSchema})
    } catch (error) {
  ...
  async search ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: StrongPointSearchSchema})
    } catch (error) {
      const msg = getErrors(error)
      // console.log(error.messages.errors)
      return response

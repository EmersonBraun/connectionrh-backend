...
import ArticlesRepository from 'App/Repositories/ArticlesRepository'
import { getErrors } from 'App/Services/MessageErros'
import { ArticleSchema, ArticleSearchSchema } from 'App/Validators'
...
export default class ManageArticlesController {
  ...
  async store ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: ArticleSchema})
    } catch (error) {
  ...
  async search ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: ArticleSearchSchema})
    } catch (error) {
      const msg = getErrors(error)
      // console.log(error.messages.errors)
      return response

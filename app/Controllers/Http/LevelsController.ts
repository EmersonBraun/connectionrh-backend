...
import LevelsRepository from 'App/Repositories/LevelsRepository'
import { getErrors } from 'App/Services/MessageErros'
import { LevelSchema, LevelSearchSchema } from 'App/Validators'

export default class LevelsController {
  private readonly repository
  constructor () {
    this.repository = LevelsRepository
  }
  ...
  async store ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: LevelSchema})
    } catch (error) {
  ...
  async search ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: LevelSearchSchema})
    } catch (error) {
      const msg = getErrors(error)
      // console.log(error.messages.errors)
      return response

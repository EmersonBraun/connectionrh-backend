...
import { RhUserSchema } from 'App/Validators'
import { UserUpdateSchema } from 'App/Validators/UserUpdateSchema'

export default class RhUsersController {
  private readonly repository
  constructor () {
    this.repository = RhUsersRepository
  }
  ...
  async store ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: RhUserSchema})
    } catch (error) {
    ...
    const { email, name, password, companyId } = request.all()
    const userData = await this.repository.firstOrCreate(email, companyId, { email, name, password })

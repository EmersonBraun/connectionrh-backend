...
import { ContractTypeSchema, ContractTypeSearchSchema } from 'App/Validators'

export default class ContractsController {
  private readonly repository
  constructor () {
    this.repository = ContractTypesRepository
  }
  ...
  async search ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: ContractTypeSearchSchema})
    } catch (error) {
      const msg = getErrors(error)
      // console.log(error.messages.errors)
      return response

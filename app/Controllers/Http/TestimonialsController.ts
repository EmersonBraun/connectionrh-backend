...
import { TestimonialSchema, TestimonialSearchSchema } from 'App/Validators'
...
export default class TestimonialsController {
  ...
  async store ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: TestimonialSchema})
    } catch (error) {
  ...
  async search ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: TestimonialSearchSchema})
    } catch (error) {
      const msg = getErrors(error)
      // console.log(error.messages.errors)
      return response

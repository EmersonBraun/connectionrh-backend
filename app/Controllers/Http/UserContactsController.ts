...
import UserContactsRepository from '../../Repositories/UserContactsRepository'
import { UserContactSchema, UserContactSearchSchema } from '../../Validators'

export default class UserContactsController {
  private readonly repository
  constructor () {
    this.repository = UserContactsRepository
  }
  ...
  async store ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: UserContactSchema})
    } catch (error) {
  ...
  async search ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: UserContactSearchSchema})
    } catch (error) {
      const msg = error.messages.errors.map(e => `${e.field} is ${e.rule}`).join(', ')
      // console.log(error.messages.errors)
      return response
